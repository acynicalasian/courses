...
int main(int argc, const char* argv[])
{
  ...
  clock_gettime(CLOCK_MONOTONIC, &start);
  sequential_convolution(padded_original_img, gblur_3x3_kernel, sequential_modified_img);
  clock_gettime(CLOCK_MONOTONIC, &finish);
  ...
  clock_gettime(CLOCK_MONOTONIC, &start);
  parallel_convolution(padded_original_img, gblur_3x3_kernel, parallel_modified_img);
  clock_gettime(CLOCK_MONOTONIC, &finish);
