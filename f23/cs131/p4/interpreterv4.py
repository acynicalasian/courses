...
class Interpreter(InterpreterBase):
    ...
    __RE_LH = r"(?<lh>(?<us1>_+)(?(us1)[a-zA-Z0-9]+|[a-zA-Z]+)\w*)\."
    __RE_RH = r"(?<rh>(?<us2>_+)(?(us2)[a-zA-Z0-9]+|[a-zA-Z]+)\w*)"
    ...
    def __call_method(self, call_ast):
        ...
        # I don't think we need to use self.__get_func_by_name bc we can't set obj fields to overloaded fns, thank god
            ...
            target_closure = objref.fields[m_name]
            target_ast = target_closure.func_ast
    ...
    def __assign(self, assign_ast):
        ...
        if objcheck:
            ...
            if checkObjExists is None:
                ...
                if fieldname == "proto" and not isinstance(src_value_obj, Object):
                    if isinstance(src_value_obj, Value):
                        if src_value_obj.v is not None:
                            super().error(ErrorType.TYPE_ERROR, "Can't set proto to non-object type")
                    else:
                        super().error(ErrorType.TYPE_ERROR, "Can't set proto to non-object type")
                checkObjExists.fields[fieldname] = src_value_obj
        else:
    ...
    def __eval_name(self, name_ast):
        ...
        if objcheck:
            ...
            if checkObjExists is None:
                ...
                super().error(ErrorType.NAME_ERROR, "Dot operator used on non-object " + objname)
            if fieldname in checkObjExists.fields:
                return checkObjExists.fields[fieldname]
            elif checkObjExists.proto is not None:
                protoptr = checkObjExists.proto
                while protoptr:
